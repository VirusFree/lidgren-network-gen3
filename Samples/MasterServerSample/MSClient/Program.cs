...
namespace MSClient
{
    static class Program
    {
        ...
        public static IPEndPoint m_hostExternal;
                            ...
                            else
                            {
                                switch ((MasterServerMessageType)inc.ReadByte())
                                {
                                    case MasterServerMessageType.Heil:
                                        MessageBox.Show("Received hello from server!");
                                        break;
        ...
        public static void SendHello()
        {
            ...
            NativeMethods.AppendText(m_mainForm.richTextBox1, "Sending hello to " + m_hostExternal);

            NetOutgoingMessage om = m_client.CreateMessage();
            om.Write((byte)MasterServerMessageType.Heil);
            om.Write("hello from client!");
            m_client.SendUnconnectedMessage(om, m_hostExternal);
        }
    }
}
